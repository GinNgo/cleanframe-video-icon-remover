...
- Set `N8N_HOST`, `N8N_PROTOCOL`, and `WEBHOOK_URL` in a private `.env` file; never commit credentials.
- Put TLS/reverse proxy in front of n8n and use secure cookies.
- Pin or review the n8n image tag before production rollout and back up the named `n8n_data` volume.
